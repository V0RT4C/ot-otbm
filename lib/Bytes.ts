import { OTBM_NODE_SPECIAL_BYTE } from '../const.ts';

export class Byt3s extends Uint8Array {
    public position = 0;

    public readByte(incramentPosition = true) {
        const val = this[incramentPosition ? this.position++ : this.position] & 0xFF;
        return val;
    }
    ...
    public escapePeekUint16LE(offset?: number) {
        const int1 : number = this._getEscapedValue(offset, false);
        const int2 : number = this._getEscapedValue(offset ? offset + 1 : this.position + 1, false);
    ...
    public escapeWriteByte(value : number) : void {
        if (
            value === OTBM_NODE_SPECIAL_BYTE.START
            ||
            value === OTBM_NODE_SPECIAL_BYTE.END
            ||
            value === OTBM_NODE_SPECIAL_BYTE.ESCAPE_CHAR){
                this.writeByte(OTBM_NODE_SPECIAL_BYTE.ESCAPE_CHAR);
    ...
    private _getEscapedValue(offset? : number, incramentPosition = true) : number {
        const backupPosition = this.position;
        this.position = (offset || offset === 0) ? offset : this.position;
        if (this[this.position] === OTBM_NODE_SPECIAL_BYTE.ESCAPE_CHAR && (this[this.position+1] === OTBM_NODE_SPECIAL_BYTE.START || this[this.position+1] === OTBM_NODE_SPECIAL_BYTE.END || this[this.position+1] === OTBM_NODE_SPECIAL_BYTE.ESCAPE_CHAR)){
            this.readByte(incramentPosition);
            const val = this.readByte(incramentPosition);
            this.position = incramentPosition ? this.position : backupPosition;
            return val;
        }else{
            const val = this.readByte(incramentPosition);
            this.position = incramentPosition ? this.position : backupPosition;
